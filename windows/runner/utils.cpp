...
constexpr const wchar_t kWindowBoundsRegKey[] =
    L"Software\\talkSPORT Companion";
...
void RestoreWindowBounds(HWND window) {
  ...
  DWORD bounds_size = sizeof(bounds);

  const LSTATUS result = ::RegGetValueW(
      HKEY_CURRENT_USER, kWindowBoundsRegKey, kWindowBoundsRegValue,
      RRF_RT_REG_BINARY, nullptr, &bounds, &bounds_size);
  RECT rect{bounds.left, bounds.top, bounds.right, bounds.bottom};
  if (result != ERROR_SUCCESS || bounds_size != sizeof(bounds) ||
...
void SaveWindowBounds(HWND window) {
  ...
  if (::IsZoomed(window) && ::GetWindowPlacement(window, &placement)) {
    bounds = placement.rcNormalPosition;
  } else if (!::GetWindowRect(window, &bounds)) {
    return;
  }

  SavedWindowBounds saved_bounds{bounds.left, bounds.top, bounds.right,
                                 bounds.bottom};
  ...
  ::RegSetValueExW(key, kWindowBoundsRegValue, 0, REG_BINARY,
                   reinterpret_cast<const BYTE*>(&saved_bounds),
                   sizeof(saved_bounds));
  ::RegCloseKey(key);
}
